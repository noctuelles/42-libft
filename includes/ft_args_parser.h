...
/*   Updated: 2024/01/16 09:55:32 by plouvel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef int (*arg_parser_parse_fn)(const char *, t_parser_state *, void *);

/**
 * The t_arg_parser_option_entry_t structure is used to describe an option.
 *
 ...
 */
typedef struct s_arg_parser_option_entry {
    const char *short_key;
    const char *long_key;
    const char *description;
    const char *argument_description;
    bool argument;
    arg_parser_parse_fn argument_parse_fn;
} t_arg_parser_option_entry;

/**
 * The t_arg_parser_config structure is used to configure the arg_parser function.
 *
 ...
 */
typedef struct s_arg_parser_config {
    char **argv;
    size_t argc;
    t_arg_parser_option_entry *parser_entries;
    size_t nbr_parser_entries;
    arg_parser_parse_fn parse_argument_fn;
    void *input;
} t_arg_parser_config;

int ft_args_parser(t_arg_parser_config *parser_config);
